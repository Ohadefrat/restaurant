  ...
  const renderTables = () => {
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      const tableReservations = reservations.filter(
        (reservation: { tableNumber: string; area: string }) =>
          reservation?.tableNumber === i.toString() &&
          reservation?.area === area,
      );
      const reservedHours = tableReservations.map(
        (reservation: { reservationTime: any }) => reservation?.reservationTime,
      );

      // Check if all allowed hours for the table are reserved
      const allHoursReserved = allowedHours.every((hour) =>
